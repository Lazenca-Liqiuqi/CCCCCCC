...
1. Use an embedding model to convert these chunks into vector embeddings that encode meaning;
1. Store these embeddings in a vector database that allows for searching by semantic similarity.
...
1. Break down the knowledge base (the "corpus" of documents) into smaller chunks of text, usually no more than a few hundred tokens;
1. Create TF-IDF encodings and semantic embeddings for these chunks;
1. Use BM25 to find top chunks based on exact matches;
1. Use embeddings to find top chunks based on semantic similarity;
1. Combine and deduplicate results from (3) and (4) using rank fusion techniques;
1. Add the top-K chunks to the prompt to generate the response.
...
![](https%3A%2F%2Fwww-cdn.anthropic.com%2Fimages%2F4zrzovbb%2Fwebsite%2F45603646e979c62349ce27744a940abf30200d57-3840x2160.png)
...
![](https%3A%2F%2Fwww-cdn.anthropic.com%2Fimages%2F4zrzovbb%2Fwebsite%2F2496e7c6fedd7ffaa043895c23a4089638b0c21b-3840x2160.png)
...
- __Contextual Embeddings reduced the top-20-chunk retrieval failure rate by 35%__(5.7% → 3.7%).
- __Combining Contextual Embeddings and Contextual BM25 reduced the top-20-chunk retrieval failure rate by 49%__(5.7% → 2.9%).

![](https%3A%2F%2Fwww-cdn.anthropic.com%2Fimages%2F4zrzovbb%2Fwebsite%2F7f8d739e491fe6b3ba0e6a9c74e4083d760b88c9-3840x2160.png)
...
1. __Chunk boundaries:__Consider how you split your documents into chunks. The choice of chunk size, chunk boundary, and chunk overlap can affect retrieval performance 1 .
1. __Embedding model:__Whereas Contextual Retrieval improves performance across all embedding models we tested, some models may benefit more than others. We found[Gemini](https://ai.google.dev/gemini-api/docs/embeddings) and[Voyage](https://www.voyageai.com/) embeddings to be particularly effective.
1. __Custom contextualizer prompts:__While the generic prompt we provided works well, you may be able to achieve even better results with prompts tailored to your specific domain or use case (for example, including a glossary of key terms that might only be defined in other documents in the knowledge base).
...
1. Pass the top-N chunks, along with the user's query, through the reranking model;
1. Using a reranking model, give each chunk a score based on its relevance and importance to the prompt, then select the top-K chunks (we used the top 20);
1. Pass the top-K chunks into the model as context to generate the final result.

![](https%3A%2F%2Fwww-cdn.anthropic.com%2Fimages%2F4zrzovbb%2Fwebsite%2F8f82c6175a64442ceff4334b54fac2ab3436a1d1-3840x2160.png)
...
![](https%3A%2F%2Fwww-cdn.anthropic.com%2Fimages%2F4zrzovbb%2Fwebsite%2F93a70cfbb7cca35bb8d86ea0a23bdeeb699e8e58-3840x2160.png)
...
1. Voyage and Gemini have the best embeddings of the ones we tested;
1. Passing the top-20 chunks to the model is more effective than just the top-10 or top-5;
1. Adding context to chunks improves retrieval accuracy a lot;
1. Reranking is better than no reranking;
1. __All these benefits stack__: to maximize performance improvements, we can combine contextual embeddings (from Voyage or Gemini) with contextual BM25, plus a reranking step, and adding the 20 chunks to the prompt.
...
![](https%3A%2F%2Fwww-cdn.anthropic.com%2Fimages%2F4zrzovbb%2Fwebsite%2F646a894ec4e6120cade9951a362f685cd2ec89b2-2458x2983.png)
