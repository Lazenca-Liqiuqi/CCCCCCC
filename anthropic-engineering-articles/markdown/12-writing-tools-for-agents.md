...
![](/_next/image?url=https%3A%2F%2Fwww-cdn.anthropic.com%2Fimages%2F4zrzovbb%2Fwebsite%2Fcdc027ad2730e4732168bb198fc9363678544f99-1920x1080.png&amp;w=3840&amp;q=75)
...
When we traditionally write software, we're establishing a contract between deterministic systems. For instance, a function call like `getWeather(“NYC”)`{.InlineCodeBlock-module-scss-module__nsPAba__code} will always fetch the weather in New York City in the exact same manner every time it is called.
...
To connect your local MCP server to Claude Code, run `claude mcp add <name> <command> [args...]`{.InlineCodeBlock-module-scss-module__nsPAba__code}.

To connect your local MCP server or DXT to the Claude Desktop app, navigate to `Settings > Developer`{.InlineCodeBlock-module-scss-module__nsPAba__code} or `Settings > Extensions`{.InlineCodeBlock-module-scss-module__nsPAba__code}, respectively.
...
![](/_next/image?url=https%3A%2F%2Fwww-cdn.anthropic.com%2Fimages%2F4zrzovbb%2Fwebsite%2F6e810aee67f3f3c955832fb7bf9033ffb0102000-1920x1080.png&amp;w=3840&amp;q=75)
...
-   Search the payment logs for `purchase_complete`{.InlineCodeBlock-module-scss-module__nsPAba__code} and `customer_id=9182`{.InlineCodeBlock-module-scss-module__nsPAba__code}.
...
We recommend running your evaluation programmatically with direct LLM API calls. Use simple agentic loops (`while`{.InlineCodeBlock-module-scss-module__nsPAba__code}-loops wrapping alternating LLM API and tool calls): one loop for each evaluation task. Each evaluation agent should be given a single task prompt and your tools.
...
![](/_next/image?url=https%3A%2F%2Fwww-cdn.anthropic.com%2Fimages%2F4zrzovbb%2Fwebsite%2F3f1f47e80974750cd924bc51e42b6df1ad997fab-1920x1080.png&amp;w=3840&amp;q=75)
...
Analyze your tool calling metrics. Lots of redundant tool calls might suggest some rightsizing of pagination or token limit parameters is warranted; lots of tool errors for invalid parameters might suggest tools could use clearer descriptions or better examples. When we launched Claude's [web search tool](https://www.anthropic.com/news/web-search), we identified that Claude was needlessly appending `2025`{.InlineCodeBlock-module-scss-module__nsPAba__code} to the tool's `query`{.InlineCodeBlock-module-scss-module__nsPAba__code} parameter, biasing search results and degrading performance (we steered Claude in the right direction by improving the tool description).
...
We recommend building a few thoughtful tools targeting specific high-impact workflows, which match your evaluation tasks and scaling up from there. In the address book case, you might choose to implement a `search_contacts`{.InlineCodeBlock-module-scss-module__nsPAba__code} or `message_contact`{.InlineCodeBlock-module-scss-module__nsPAba__code} tool instead of a `list_contacts`{.InlineCodeBlock-module-scss-module__nsPAba__code} tool.
...
-   Instead of implementing a `list_users`{.InlineCodeBlock-module-scss-module__nsPAba__code}, `list_events`{.InlineCodeBlock-module-scss-module__nsPAba__code}, and `create_event`{.InlineCodeBlock-module-scss-module__nsPAba__code} tools, consider implementing a `schedule_event`{.InlineCodeBlock-module-scss-module__nsPAba__code} tool which finds availability and schedules an event.
-   Instead of implementing a `read_logs`{.InlineCodeBlock-module-scss-module__nsPAba__code} tool, consider implementing a `search_logs`{.InlineCodeBlock-module-scss-module__nsPAba__code} tool which only returns relevant log lines and some surrounding context.
-   Instead of implementing `get_customer_by_id`{.InlineCodeBlock-module-scss-module__nsPAba__code}, `list_transactions`{.InlineCodeBlock-module-scss-module__nsPAba__code}, and `list_notes`{.InlineCodeBlock-module-scss-module__nsPAba__code} tools, implement a `get_customer_context`{.InlineCodeBlock-module-scss-module__nsPAba__code} tool which compiles all of a customer's recent & relevant information all at once.
...
Namespacing (grouping related tools under common prefixes) can help delineate boundaries between lots of tools; MCP clients sometimes do this by default. For example, namespacing tools by service (e.g., `asana_search`{.InlineCodeBlock-module-scss-module__nsPAba__code}, `jira_search`{.InlineCodeBlock-module-scss-module__nsPAba__code}) and by resource (e.g., `asana_projects_search`{.InlineCodeBlock-module-scss-module__nsPAba__code}, `asana_users_search`{.InlineCodeBlock-module-scss-module__nsPAba__code}), can help agents select the right tools at the right time.
...
In the same vein, tool implementations should take care to return only high signal information back to agents. They should prioritize contextual relevance over flexibility, and eschew low-level technical identifiers (for example: `uuid`{.InlineCodeBlock-module-scss-module__nsPAba__code}, `256px_image_url`{.InlineCodeBlock-module-scss-module__nsPAba__code}, `mime_type`{.InlineCodeBlock-module-scss-module__nsPAba__code}). Fields like `name`{.InlineCodeBlock-module-scss-module__nsPAba__code}, `image_url`{.InlineCodeBlock-module-scss-module__nsPAba__code}, and `file_type`{.InlineCodeBlock-module-scss-module__nsPAba__code} are much more likely to directly inform agents' downstream actions and responses.
...
In some instances, agents may require the flexibility to interact with both natural language and technical identifiers outputs, if only to trigger downstream tool calls (for example, `search_user(name=’jane’)`{.InlineCodeBlock-module-scss-module__nsPAba__code} → `send_message(id=12345)`{.InlineCodeBlock-module-scss-module__nsPAba__code}). You can enable both by exposing a simple `response_format`{.InlineCodeBlock-module-scss-module__nsPAba__code} enum parameter in your tool, allowing your agent to control whether tools return `“concise”`{.InlineCodeBlock-module-scss-module__nsPAba__code} or `“detailed”`{.InlineCodeBlock-module-scss-module__nsPAba__code} responses (images below).
...
![](/_next/image?url=https%3A%2F%2Fwww-cdn.anthropic.com%2Fimages%2F4zrzovbb%2Fwebsite%2Fd4f649a66482efb5a80cf14ea85e84974ede1c49-1920x725.png&amp;w=3840&amp;q=75)
Slack threads and thread replies are identified by unique <code>thread_ts</code> which are required to fetch thread replies. <code>thread_ts</code> and other IDs (<code>channel_id</code>, <code>user_id</code>) can be retrieved from a <code>“detailed”</code> tool response to enable further tool calls that require these. <code>“concise”</code> tool responses return only thread content and exclude IDs. In this example, we use ~⅓ of the tokens with <code>“concise”</code> tool responses.
...
![](/_next/image?url=https%3A%2F%2Fwww-cdn.anthropic.com%2Fimages%2F4zrzovbb%2Fwebsite%2F810661bd44a35fb273806ae95160040155978c3e-1920x850.png&amp;w=3840&amp;q=75)
...
When writing tool descriptions and specs, think of how you would describe your tool to a new hire on your team. Consider the context that you might implicitly bring---specialized query formats, definitions of niche terminology, relationships between underlying resources---and make it explicit. Avoid ambiguity by clearly describing (and enforcing with strict data models) expected inputs and outputs. In particular, input parameters should be unambiguously named: instead of a parameter named `user`{.InlineCodeBlock-module-scss-module__nsPAba__code}, try a parameter named `user_id`{.InlineCodeBlock-module-scss-module__nsPAba__code}.
