...
![](/_next/image?url=https%3A%2F%2Fwww-cdn.anthropic.com%2Fimages%2F4zrzovbb%2Fwebsite%2Ffaa261102e46c7f090a2402a49000ffae18c5dd6-2292x1290.png&amp;w=3840&amp;q=75)
<em>In contrast to the discrete task of writing a prompt, context engineering is iterative and the curation phase happens each time we decide what to pass to the model.</em>
...
![](/_next/image?url=https%3A%2F%2Fwww-cdn.anthropic.com%2Fimages%2F4zrzovbb%2Fwebsite%2F0442fe138158e84ffce92bed1624dd09f37ac46f-2292x1288.png&amp;w=3840&amp;q=75)
<em>At one end of the spectrum, we see brittle if-else hardcoded prompts, and at the other end we see prompts that are overly general or falsely assume shared context.</em>

We recommend organizing prompts into distinct sections (like `<background_information>`{.InlineCodeBlock-module-scss-module__nsPAba__code}, `<instructions>`{.InlineCodeBlock-module-scss-module__nsPAba__code}, `## Tool guidance`{.InlineCodeBlock-module-scss-module__nsPAba__code}, `## Output description`{.InlineCodeBlock-module-scss-module__nsPAba__code}, etc) and using techniques like XML tagging or Markdown headers to delineate these sections, although the exact formatting of prompts is likely becoming less important as models become more capable.
...
Beyond storage efficiency, the metadata of these references provides a mechanism to efficiently refine behavior, whether explicitly provided or intuitive. To an agent operating in a file system, the presence of a file named `test_utils.py`{.InlineCodeBlock-module-scss-module__nsPAba__code} in a `tests`{.InlineCodeBlock-module-scss-module__nsPAba__code} folder implies a different purpose than a file with the same name located in `src/core_logic/`{.InlineCodeBlock-module-scss-module__nsPAba__code} Folder hierarchies, naming conventions, and timestamps all provide important signals that help both humans and agents understand how and when to utilize information.
