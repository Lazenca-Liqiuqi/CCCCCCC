...
*Looking for more detailed information? Our comprehensive documentation at [claude.ai/code](https://claude.ai/redirect/website.v1.08ca5b0a-3c50-4742-8aad-89eec8ae81fb/code)* *covers all the features mentioned in this post and provides additional examples, implementation details, and advanced techniques.*
...
### a. Create `CLAUDE.md`{.InlineCodeBlock-module-scss-module__nsPAba__code} files 
`CLAUDE.md`{.InlineCodeBlock-module-scss-module__nsPAba__code} is a special file that Claude automatically pulls into context when starting a conversation. This makes it an ideal place for documenting:
...
There's no required format for `CLAUDE.md`{.InlineCodeBlock-module-scss-module__nsPAba__code} files. We recommend keeping them concise and human-readable. For example:
...
You can place `CLAUDE.md`{.InlineCodeBlock-module-scss-module__nsPAba__code} files in several locations:

-   **The root of your repo**, or wherever you run `claude`{.InlineCodeBlock-module-scss-module__nsPAba__code} from (the most common usage). Name it `CLAUDE.md`{.InlineCodeBlock-module-scss-module__nsPAba__code} and check it into git so that you can share it across sessions and with your team (recommended), or name it `CLAUDE.local.md`{.InlineCodeBlock-module-scss-module__nsPAba__code} and `.gitignore`{.InlineCodeBlock-module-scss-module__nsPAba__code} it
-   **Any parent of the directory** where you run `claude`{.InlineCodeBlock-module-scss-module__nsPAba__code}. This is most useful for monorepos, where you might run `claude`{.InlineCodeBlock-module-scss-module__nsPAba__code} from `root/foo`{.InlineCodeBlock-module-scss-module__nsPAba__code}, and have `CLAUDE.md`{.InlineCodeBlock-module-scss-module__nsPAba__code} files in both `root/CLAUDE.md`{.InlineCodeBlock-module-scss-module__nsPAba__code} and `root/foo/CLAUDE.md`{.InlineCodeBlock-module-scss-module__nsPAba__code}. Both of these will be pulled into context automatically
-   **Any child of the directory** where you run `claude`{.InlineCodeBlock-module-scss-module__nsPAba__code}. This is the inverse of the above, and in this case, Claude will pull in `CLAUDE.md`{.InlineCodeBlock-module-scss-module__nsPAba__code} files on demand when you work with files in child directories
-   **Your home folder** (`~/.claude/CLAUDE.md`{.InlineCodeBlock-module-scss-module__nsPAba__code}), which applies it to all your *claude* sessions

When you run the `/init`{.InlineCodeBlock-module-scss-module__nsPAba__code} command, Claude will automatically generate a `CLAUDE.md`{.InlineCodeBlock-module-scss-module__nsPAba__code} for you.

### b. Tune your `CLAUDE.md`{.InlineCodeBlock-module-scss-module__nsPAba__code} files 
Your `CLAUDE.md`{.InlineCodeBlock-module-scss-module__nsPAba__code} files become part of Claude's prompts, so they should be refined like any frequently used prompt. A common mistake is adding extensive content without iterating on its effectiveness. Take time to experiment and determine what produces the best instruction following from the model.

You can add content to your `CLAUDE.md`{.InlineCodeBlock-module-scss-module__nsPAba__code} manually or press the `#`{.InlineCodeBlock-module-scss-module__nsPAba__code} key to give Claude an instruction that it will automatically incorporate into the relevant `CLAUDE.md`{.InlineCodeBlock-module-scss-module__nsPAba__code}. Many engineers use `#`{.InlineCodeBlock-module-scss-module__nsPAba__code} frequently to document commands, files, and style guidelines while coding, then include `CLAUDE.md`{.InlineCodeBlock-module-scss-module__nsPAba__code} changes in commits so team members benefit as well.

At Anthropic, we occasionally run `CLAUDE.md`{.InlineCodeBlock-module-scss-module__nsPAba__code} files through the [prompt improver](https://docs.anthropic.com/en/docs/build-with-claude/prompt-engineering/prompt-improver) and often tune instructions (e.g. adding emphasis with \"IMPORTANT\" or \"YOU MUST\") to improve adherence.

![](/_next/image?url=https%3A%2F%2Fwww-cdn.anthropic.com%2Fimages%2F4zrzovbb%2Fwebsite%2F6961243cc6409e41ba93895faded4f4bc1772366-1600x1231.png&amp;w=3840&amp;q=75)

### c. Curate Claude\'s list of allowed tools 
By default, Claude Code requests permission for any action that might modify your system: file writes, many bash commands, MCP tools, etc. We designed Claude Code with this deliberately conservative approach to prioritize safety. You can customize the allowlist to permit additional tools that you know are safe, or to allow potentially unsafe tools that are easy to undo (e.g., file editing, `git commit`{.InlineCodeBlock-module-scss-module__nsPAba__code}).

There are four ways to manage allowed tools:

-   **Select \"Always allow\"** when prompted during a session.
-   **Use the `/permissions`{.InlineCodeBlock-module-scss-module__nsPAba__code} command** after starting Claude Code to add or remove tools from the allowlist. For example, you can add `Edit`{.InlineCodeBlock-module-scss-module__nsPAba__code} to always allow file edits, `Bash(git commit:*)`{.InlineCodeBlock-module-scss-module__nsPAba__code} to allow git commits, or `mcp__puppeteer__puppeteer_navigate`{.InlineCodeBlock-module-scss-module__nsPAba__code} to allow navigating with the Puppeteer MCP server.
-   **Manually edit** your `.claude/settings.json`{.InlineCodeBlock-module-scss-module__nsPAba__code} or `~/.claude.json`{.InlineCodeBlock-module-scss-module__nsPAba__code} (we recommend checking the former into source control to share with your team)*.*
-   **Use the` --allowedTools`{.InlineCodeBlock-module-scss-module__nsPAba__code} CLI flag** for session-specific permissions.

### d. If using GitHub, install the gh CLI 
Claude knows how to use the `gh`{.InlineCodeBlock-module-scss-module__nsPAba__code} CLI to interact with GitHub for creating issues, opening pull requests, reading comments, and more. Without `gh`{.InlineCodeBlock-module-scss-module__nsPAba__code} installed, Claude can still use the GitHub API or MCP server (if you have it installed).
...
Claude Code inherits your bash environment, giving it access to all your tools. While Claude knows common utilities like unix tools and `gh`{.InlineCodeBlock-module-scss-module__nsPAba__code}, it won\'t know about your custom bash tools without instructions:

1.  Tell Claude the tool name with usage examples
2.  Tell Claude to run `--help`{.InlineCodeBlock-module-scss-module__nsPAba__code} to see tool documentation
3.  Document frequently used tools in `CLAUDE.md`{.InlineCodeBlock-module-scss-module__nsPAba__code}
...
-   **In a checked-in `.mcp.json`{.InlineCodeBlock-module-scss-module__nsPAba__code} file** (available to anyone working in your codebase). For example, you can add Puppeteer and Sentry servers to your `.mcp.json`{.InlineCodeBlock-module-scss-module__nsPAba__code}, so that every engineer working on your repo can use these out of the box.

When working with MCP, it can also be helpful to launch Claude with the `--mcp-debug`{.InlineCodeBlock-module-scss-module__nsPAba__code} flag to help identify configuration issues.

### c. Use custom slash commands 
For repeated workflows---debugging loops, log analysis, etc.---store prompt templates in Markdown files within the `.claude/commands`{.InlineCodeBlock-module-scss-module__nsPAba__code} folder. These become available through the slash commands menu when you type `/`{.InlineCodeBlock-module-scss-module__nsPAba__code}. You can check these commands into git to make them available for the rest of your team.

Custom slash commands can include the special keyword `$ARGUMENTS`{.InlineCodeBlock-module-scss-module__nsPAba__code} to pass parameters from command invocation.
...
Putting the above content into `.claude/commands/fix-github-issue.md`{.InlineCodeBlock-module-scss-module__nsPAba__code} makes it available as the `/project:fix-github-issue`{.InlineCodeBlock-module-scss-module__nsPAba__code} command in Claude Code. You could then for example use `/project:fix-github-issue 1234`{.InlineCodeBlock-module-scss-module__nsPAba__code} to have Claude fix issue #1234. Similarly, you can add your own personal commands to the `~/.claude/commands`{.InlineCodeBlock-module-scss-module__nsPAba__code} folder for commands you want available in all of your sessions.
...
![](/_next/image?url=https%3A%2F%2Fwww-cdn.anthropic.com%2Fimages%2F4zrzovbb%2Fwebsite%2F6ea59a36fe82c2b300bceaf3b880a4b4852c552d-1600x1143.png&amp;w=3840&amp;q=75)

### d. Safe YOLO mode 
Instead of supervising Claude, you can use `claude --dangerously-skip-permissions`{.InlineCodeBlock-module-scss-module__nsPAba__code} to bypass all permission checks and let Claude work uninterrupted until completion. This works well for workflows like fixing lint errors or generating boilerplate code.

Letting Claude run arbitrary commands is risky and can result in data loss, system corruption, or even data exfiltration (e.g., via prompt injection attacks). To minimize these risks, use `--dangerously-skip-permissions`{.InlineCodeBlock-module-scss-module__nsPAba__code} in a container without internet access. You can follow this [reference implementation](https://github.com/anthropics/claude-code/tree/main/.devcontainer) using Docker Dev Containers.
...
-   What does `async move { ... }`{.InlineCodeBlock-module-scss-module__nsPAba__code} do on line 134 of `foo.rs`{.InlineCodeBlock-module-scss-module__nsPAba__code}?
-   What edge cases does `CustomerOnboardingFlowImpl`{.InlineCodeBlock-module-scss-module__nsPAba__code} handle?
-   Why are we calling `foo()`{.InlineCodeBlock-module-scss-module__nsPAba__code} instead of `bar()`{.InlineCodeBlock-module-scss-module__nsPAba__code} on line 333?
-   What's the equivalent of line 334 of `baz.py`{.InlineCodeBlock-module-scss-module__nsPAba__code} in Java?
...
![](/_next/image?url=https%3A%2F%2Fwww-cdn.anthropic.com%2Fimages%2F4zrzovbb%2Fwebsite%2Fa08ea13c2359aac0eceacebf2e15f81e8e8ec8d2-1600x1278.png&amp;w=3840&amp;q=75)
...
This eliminates the need to remember `gh`{.InlineCodeBlock-module-scss-module__nsPAba__code} command line syntax while automating routine tasks.

### h. Use Claude to work with Jupyter notebooks 
Researchers and data scientists at Anthropic use Claude Code to read and write Jupyter notebooks. Claude can interpret outputs, including images, providing a fast way to explore and interact with data. There are no required prompts or workflows, but a workflow we recommend is to have Claude Code and a `.ipynb`{.InlineCodeBlock-module-scss-module__nsPAba__code} file open side-by-side in VS Code.
...
![](/_next/image?url=https%3A%2F%2Fwww-cdn.anthropic.com%2Fimages%2F4zrzovbb%2Fwebsite%2F75e1b57a0b696e7aafeca1ed5fa6ba7c601a5953-1360x1126.png&amp;w=3840&amp;q=75)
...
![](/_next/image?url=https%3A%2F%2Fwww-cdn.anthropic.com%2Fimages%2F4zrzovbb%2Fwebsite%2F7372868757dd17b6f2d3fef98d499d7991d89800-1450x1164.png&amp;w=3840&amp;q=75)
...
![](/_next/image?url=https%3A%2F%2Fwww-cdn.anthropic.com%2Fimages%2F4zrzovbb%2Fwebsite%2Fe071de707f209bbaa7f16b593cc7ed0739875dae-1306x1088.png&amp;w=3840&amp;q=75)

### d. Give Claude URLs 
Paste specific URLs alongside your prompts for Claude to fetch and read. To avoid permission prompts for the same domains (e.g., docs.foo.com), use `/permissions`{.InlineCodeBlock-module-scss-module__nsPAba__code} to add domains to your allowlist.
...
### f. Use `/clear`{.InlineCodeBlock-module-scss-module__nsPAba__code} to keep context focused 
During long sessions, Claude\'s context window can fill with irrelevant conversation, file contents, and commands. This can reduce performance and sometimes distract Claude. Use the `/clear`{.InlineCodeBlock-module-scss-module__nsPAba__code} command frequently between tasks to reset the context window.
...
-   **Pipe into Claude Code** (e.g., `cat foo.txt | claude`{.InlineCodeBlock-module-scss-module__nsPAba__code}), particularly useful for logs, CSVs, and large data
...
Claude Code includes [headless mode](https://docs.anthropic.com/en/docs/agents-and-tools/claude-code/overview#automate-ci-and-infra-workflows) for non-interactive contexts like CI, pre-commit hooks, build scripts, and automation. Use the `-p`{.InlineCodeBlock-module-scss-module__nsPAba__code} flag with a prompt to enable headless mode, and `--output-format stream-json`{.InlineCodeBlock-module-scss-module__nsPAba__code} for streaming JSON output.
...
2.  Run `/clear`{.InlineCodeBlock-module-scss-module__nsPAba__code} or start a second Claude in another terminal
3.  Have the second Claude review the first Claude\'s work
4.  Start another Claude (or `/clear`{.InlineCodeBlock-module-scss-module__nsPAba__code} again) to read both the code and review feedback
...
1.  **Create worktrees**: `git worktree add ../project-feature-a feature-a`{.InlineCodeBlock-module-scss-module__nsPAba__code}
2.  **Launch Claude in each worktree**: `cd ../project-feature-a && claude`{.InlineCodeBlock-module-scss-module__nsPAba__code}
...
-   Clean up when finished: `git worktree remove ../project-feature-a`{.InlineCodeBlock-module-scss-module__nsPAba__code}

### d. Use headless mode with a custom harness 
`claude -p`{.InlineCodeBlock-module-scss-module__nsPAba__code} (headless mode) integrates Claude Code programmatically into larger workflows while leveraging its built-in tools and system prompt. There are two primary patterns for using headless mode:
...
2.  Loop through tasks, calling Claude programmatically for each and giving it a task and a set of tools it can use. For example: `claude -p “migrate foo.py from React to Vue. When you are done, you MUST return the string OK if you succeeded, or FAIL if the task failed.” --allowedTools Edit Bash(git commit:*)`{.InlineCodeBlock-module-scss-module__nsPAba__code}
...
1.  Call `claude -p “<your prompt>” --json | your_command`{.InlineCodeBlock-module-scss-module__nsPAba__code}, where `your_command`{.InlineCodeBlock-module-scss-module__nsPAba__code} is the next step of your processing pipeline
2.  That's it! JSON output (optional) can help provide structure for easier automated processing.

For both of these use cases, it can be helpful to use the `--verbose`{.InlineCodeBlock-module-scss-module__nsPAba__code} flag for debugging the Claude invocation. We generally recommend turning verbose mode off in production for cleaner output.
...
{.Icon-module-scss-module__lqbdHG__icon .ToutCallout-module-scss-module__gZk6bG__icon}Explore courses
