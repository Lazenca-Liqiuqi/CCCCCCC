-   ::: {.body-3 .LatestUpdates-module-scss-module__sg7Vka__headline}
...
Desktop Extensions (`.mcpb`{.InlineCodeBlock-module-scss-module__nsPAba__code} files) solve these problems by bundling an entire MCP server---including all dependencies---into a single installable package. Here\'s what changes for users:
...
1.  Download a `.mcpb`{.InlineCodeBlock-module-scss-module__nsPAba__code} file
...
A Desktop Extension is a zip archive containing the local MCP server as well as a `manifest.json`{.InlineCodeBlock-module-scss-module__nsPAba__code}, which describes everything Claude Desktop and other apps supporting desktop extensions need to know.
...
Let's take a look at a concrete example of how the manifest aids with configuration. In the manifest below, the developer declares that the user needs to supply an `api_key`{.InlineCodeBlock-module-scss-module__nsPAba__code}. Claude will not enable the extension until the user has supplied that value, keep it automatically in the operating system's secret vault, and transparently replace the `${user_config.api_key}`{.InlineCodeBlock-module-scss-module__nsPAba__code} with the user-supplied value when launching the server. Similarly, `${__dirname}`{.InlineCodeBlock-module-scss-module__nsPAba__code} will be replaced with the full path to the extension's unpacked directory.
...
A full `manifest.json`{.InlineCodeBlock-module-scss-module__nsPAba__code} with most of the optional fields might look like this:
...
The full specification for all required and optional fields in the `manifest.json`{.InlineCodeBlock-module-scss-module__nsPAba__code} can be found as part of our [open-source toolchain](https://github.com/anthropics/dxt/blob/main/MANIFEST.md).
...
Bundle everything into a `.mcpb`{.InlineCodeBlock-module-scss-module__nsPAba__code} file:
...
2.  Generates the `.mcpb`{.InlineCodeBlock-module-scss-module__nsPAba__code} archive

#### Step 4: Test locally 
Drag your `.mcpb`{.InlineCodeBlock-module-scss-module__nsPAba__code} file into Claude Desktop\'s Settings window. You\'ll see:
...
-   `${__dirname}`{.InlineCodeBlock-module-scss-module__nsPAba__code}: Extension\'s installation directory
-   `${user_config.key}`{.InlineCodeBlock-module-scss-module__nsPAba__code}: User-provided configuration
-   `${HOME}, ${TEMP}`{.InlineCodeBlock-module-scss-module__nsPAba__code}: System environment variables
