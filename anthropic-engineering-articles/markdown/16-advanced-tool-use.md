...
But token cost isn\'t the only issue. The most common failures are wrong tool selection and incorrect parameters, especially when tools have similar names like `notification-send-user`{.InlineCodeBlock-module-scss-module__nsPAba__code} vs. `notification-send-channel`{.InlineCodeBlock-module-scss-module__nsPAba__code}.
...
![](/_next/image?url=https%3A%2F%2Fwww-cdn.anthropic.com%2Fimages%2F4zrzovbb%2Fwebsite%2Ff359296f770706608901eadaffbff4ca0b67874c-1999x1125.png&amp;w=3840&amp;q=75)
<em>Tool Search Tool preserves 191,300 tokens of context compared to 122,800 with Claude’s traditional approach.</em>
...
The Tool Search Tool lets Claude dynamically discover tools instead of loading all definitions upfront. You provide all your tool definitions to the API, but mark tools with `defer_loading: true`{.InlineCodeBlock-module-scss-module__nsPAba__code} to make them discoverable on-demand. Deferred tools aren\'t loaded into Claude\'s context initially. Claude only sees the Tool Search Tool itself plus any tools with `defer_loading: false`{.InlineCodeBlock-module-scss-module__nsPAba__code} (your most critical, frequently-used tools).
...
For example, if Claude needs to interact with GitHub, it searches for \"github,\" and only `github.createPullRequest`{.InlineCodeBlock-module-scss-module__nsPAba__code} and `github.listIssues`{.InlineCodeBlock-module-scss-module__nsPAba__code} get loaded---not your other 50+ tools from Slack, Jira, and Google Drive.
...
``` 
, # defer loading the entire server
  "configs":   // Keep most used tool loaded
  }
}
```
...
-   `get_team_members(department)`{.InlineCodeBlock-module-scss-module__nsPAba__code} - Returns team member list with IDs and levels
-   `get_expenses(user_id, quarter)`{.InlineCodeBlock-module-scss-module__nsPAba__code} - Returns expense line items for a user
-   `get_budget_by_level(level)`{.InlineCodeBlock-module-scss-module__nsPAba__code} - Returns budget limits for an employee level
...
![](/_next/image?url=https%3A%2F%2Fwww-cdn.anthropic.com%2Fimages%2F4zrzovbb%2Fwebsite%2F65737d69a3290ed5c1f3c3b8dc873645a9dcc2eb-1999x1491.png&amp;w=3840&amp;q=75)
...
-   **Format ambiguity:** Should `due_date`{.InlineCodeBlock-module-scss-module__nsPAba__code} use \"2024-11-06\", \"Nov 6, 2024\", or \"2024-11-06T00:00:00Z\"?
-   **ID conventions:** Is `reporter.id`{.InlineCodeBlock-module-scss-module__nsPAba__code} a UUID, \"USR-12345\", or just \"12345\"?
-   **Nested structure usage:** When should Claude populate `reporter.contact`{.InlineCodeBlock-module-scss-module__nsPAba__code}?
-   **Parameter correlations:** How do `escalation.level`{.InlineCodeBlock-module-scss-module__nsPAba__code} and `escalation.sla_hours`{.InlineCodeBlock-module-scss-module__nsPAba__code} relate to priority?
...
-   Similar tools where examples clarify which one to use (e.g., `create_ticket`{.InlineCodeBlock-module-scss-module__nsPAba__code} vs `create_incident`{.InlineCodeBlock-module-scss-module__nsPAba__code})
...
-   [[D]ocumentation](https://platform.claude.com/docs/en/agents-and-tools/tool-use/tool-search-tool) and [cookbook](https://github.com/anthropics/claude-cookbooks/blob/main/tool_use/tool_search_with_embeddings.ipynb) for Tool Search Tool
