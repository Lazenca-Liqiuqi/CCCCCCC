...
| --- | --- | --- | --- | --- | --- |
| Claude declares the entire project complete too early. | Claude 过早宣布整个项目完成。 | Set up a feature list file: create a structured JSON file with end-to-end feature descriptions based on input specifications. | 设置功能列表文件：根据输入规范，设置一个结构化的 JSON 文件，其中包含端到端功能描述列表。 | Read the feature list file at the start of the session. Choose one feature to work on. | 在会话开始时阅读功能列表文件。选择一个功能开始工作。 |
| Claude leaves the environment in a buggy state with undocumented progress. | Claude 将环境置于有错误或未记录进展的状态。 | Write initial git repo and progress notes file. | 编写初始 git 存储库和进度笔记文件。 | Start sessions by reading progress notes and git commit logs, and run basic tests on the dev server to catch any undocumented bugs. End sessions by writing git commits and progress updates. | 通过阅读进度笔记文件和 git 提交日志开始会话，并在开发服务器上运行基本测试以捕获任何未记录的错误。通过编写 git 提交和进度更新来结束会话。 |
| Claude marks features as complete too early. | Claude 过早地将功能标记为完成。 | Set up feature list file. | 设置功能列表文件。 | Self-verify all features. Only mark features as "passing" after careful testing. | 自我验证所有功能。只有在仔细测试后才将功能标记为"通过"。 |
| Claude has to spend time figuring out how to run the application. | Claude 必须花时间弄清楚如何运行应用程序。 | Write an `init.sh` script that can run the dev server. | 编写一个可以运行开发服务器的 `init.sh` 脚本。 | Start sessions by reading `init.sh`. | 通过阅读 `init.sh` 开始会话。 |
